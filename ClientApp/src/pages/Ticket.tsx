...
import {getCompany, getId} from "../Endpoints/Jwt";
    ...
    const fetchTicket = () => {
        FetchTicketAxios(TicketId)
            .then((res) => {
                ...
                const comp = getCompany(token);
                if (res.data["compId"] != comp) nav('/');
                else {
                    const {
                        company,
                        creator,
    ...
    return (
        <div className={"w-full"}>
            <div className="max-w-2xl mx-auto">
              ...
              <span
                  className="absolute top-1/2 left-1/2 text-2xl font-semibold text-white -translate-x-1/2 -translate-y-1/2 sm:text-3xl dark:text-gray-800">
                First Slide
              </span>
                            <img
